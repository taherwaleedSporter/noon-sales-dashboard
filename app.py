...
def read_stock(file) -> pd.DataFrame:
    ...
    grouped = (
        ...
        .agg(
            ...
            stock_title=("title", lambda s: s.dropna().astype(str).iloc[0] if not s.dropna().empty else None) if "title" in df.columns else ("qty", "size"),
            stock_brand=("brand", lambda s: s.dropna().astype(str).iloc[0] if not s.dropna().empty else None) if "brand" in df.columns else ("qty", "size"),
            stock_sku=("sku", lambda s: s.dropna().astype(str).iloc[0] if not s.dropna().empty else None) if "sku" in df.columns else ("qty", "size"),
        )
    )
...
# Filters
brands = sorted([str(b) for b in model["brand"].dropna().unique().tolist()])
currencies = sorted([str(c) for c in model["currency"].dropna().unique().tolist()])
...
filtered = model[
    (model["date_dt"].dt.date >= start_date) & (model["date_dt"].dt.date <= end_date)
...
with chart1:
    sales_trend = filtered.groupby("date")["sales_usd"].sum()
    st.line_chart(sales_trend)
with chart2:
    orders_trend = filtered.groupby("date")["orders"].sum()
    st.bar_chart(orders_trend)

tabs = st.tabs(["Daily", "Monthly", "Product Orders", "Product Sales", "Product Metrics", "Current Stock", "Stock Coverage", "Gainers/Decliners", "MTD", "Data Quality"])

with tabs[0]: st.dataframe(daily, use_container_width=True)
with tabs[1]: st.dataframe(monthly, use_container_width=True)
with tabs[2]: st.dataframe(product_orders, use_container_width=True)
with tabs[3]: st.dataframe(product_sales, use_container_width=True)
with tabs[4]: st.dataframe(product_metrics, use_container_width=True)
with tabs[5]: 
    if not stock_report.empty: st.dataframe(stock_report, use_container_width=True)
...
with tabs[6]:
    ...
    else: st.info("No coverage data.")

with tabs[7]:
    g1, g2 = st.columns(2)
    g1.dataframe(top_gainers)
    g2.dataframe(top_decliners)

with tabs[8]:
    if mtd:
        m1, m2, m3 = st.columns(3)
        m1.metric("MTD Sales", f"${mtd['mtd_sales']:,.2f}", f"{mtd['sales_delta']:.2%}" if mtd['sales_delta'] else None)
        m2.metric("MTD Orders", f"{mtd['mtd_orders']:,}", f"{mtd['orders_delta']:.2%}" if mtd['orders_delta'] else None)
        m3.metric("MTD GP", f"${mtd['mtd_gp']:,.2f}", f"{mtd['gp_delta']:.2%}" if mtd['gp_delta'] else None)

with tabs[9]:
    st.write("Missing COGS:", len(missing_cogs))
    st.write("Missing Fees:", len(missing_fees))
    if not missing_cogs.empty: st.dataframe(missing_cogs)
...
excel_bytes = to_excel_bytes({
    ...
    "Current Stock": stock_report,
    "Stock Coverage": stock_coverage
})

st.download_button("Download Excel Report", data=excel_bytes, file_name="noon_report.xlsx")
